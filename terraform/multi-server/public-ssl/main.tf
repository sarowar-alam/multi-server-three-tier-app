# =============================================================================
#  Case 1b: Multi-server + public IP + Let's Encrypt HTTPS
#  Access: https://bmi.ostaddevops.click
#
#  Architecture:
#    DB       -> private subnet (5432 from backend SG, NAT for setup)
#    Backend  -> private subnet (3000 from frontend SG, NAT for setup)
#    Frontend -> public subnet  (80/443 from internet, public IP)
#    Nginx: HTTP:80 redirects to HTTPS:443 (certbot --redirect).
#    Route53 plain A: domain -> frontend public IP (created by Terraform).
# =============================================================================
...
# ── VPC (identical to public-http, NAT for private instances) ─────────────────
...
# ── Security groups (identical to public-http) ────────────────────────────────

resource "aws_security_group" "frontend" {
  name = "bmi-frontend-sg"; description = "BMI Frontend: 80/443 from internet"; vpc_id = aws_vpc.main.id
...
resource "aws_instance" "frontend" {
  ami = var.ami_id; instance_type = var.instance_type; subnet_id = aws_subnet.public[0].id
  ...
  # Route53 A record must exist before instance boots for certbot HTTP-01 validation
  depends_on = [aws_route53_record.main]
  root_block_device { volume_size = 20; volume_type = "gp3"; delete_on_termination = true }
  tags = merge(local.common_tags, { Name = "bmi-frontend" })
}

# ── Route53 plain A record → frontend public IP ───────────────────────────────
